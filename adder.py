...
'''
  if dct['attending'] != '':
    ...
    course = tmp.split('-')[0]
    tp = tmp.split('-')[1]
    out.append("You are attending lecture for {} for {}\n".format(course, tp))
...
'''
  if dct['total_hours'] != '':
    out.append("You are holding {} open lab hours per week.\n\n".format(dct['total_hours']))
    if dct['12_hours'] != '':
    ...
    out.append("\nYou will hold your open lab hours in {}\n\n".format(dct['location']))
  else:
    out.append("You are not holding open lab hours.\n\n")

def add_ticket_portion(dct: ParsedRowDict, out: MessageString) -> None:
  '''Adds the Autograder ticket answering portion of the email to the MessageString.
